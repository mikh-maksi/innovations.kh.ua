	...
	<div class="table-responsive col-lg-12 col-sm-12	col-xs-12">
		...
		switch ($id){
			case 1:
				...
				<script type="text/javascript">	hide_btn_add.addEventListener("click",hide_add_t);
	function hide_add_t()
{
	...
	<div class="grfik" id = "gra">
		...
		?>
			...
			?>
				...
				<?
				$query = "SELECT * FROM ved_direct_investment WHERE year >=2010 ORDER BY year";
				$result = MYSQL_QUERY($query);
				$i=0;
				while ($out = mysql_fetch_array($result,MYSQL_ASSOC)){
				echo "<tr>";
				...
				$j=0;
				foreach ($out as $key => $value){
					if ($a==0) {$a=1; continue;} //Убираем id
					echo "<td>".$value."</td>";
...
<?php
			...
			?>
				...
				<div class="grfik" id = "gra">
				<?php 
				include ("grafiks/zed_direct_investment_countries.php");
				?>
				</div>
			...
			?>
				...
				<div class="grfik" id = "gra">
				<?php 
				include ("grafiks/zed_export_goods.php");
				?>
				</div>
			...
			?>
				...
				foreach ($out as $key => $value){
					if ($a==0) {$a = 1; continue;}
					echo "<td>".iconv("cp1251", "UTF-8", $value)."</td>";
					$arr_data[$j][$i] = iconv("cp1251", "UTF-8", $value);
					$j++;
				}
				echo "</tr>";
				$i++;	
				...
				<div class="grfik" id = "gra">
				<?php 
				include ("grafiks/zed_export_services.php");
				?>
				</div>
...
<?php			
			...
			?>
				...
				<div class="grfik" id = "gra">
				<?php 
				//include ("grafiks/zed_services.php");
				?>
				</div>
	...
	<div class="grfik" id = "gra">
		...
		<?php		
			...
			case 10:
				...
				<td>Количество заведений, осуществляющих подготовку аспирантв</td>
				<td>Принято в аспирантуру, чел</td>
				<td>Выпущено аспирантов, чел</td>
				<td>Принято в докторантуру, чел</td>
				<td>Выпущено докторантов, чел</td>
				<td>Количество заведений, осуществляющих подготовку докторантов</td>
				<td>Кол-во общеобразовательных учреждений, чел</td>
				<td>Введено в эксплуатацию общеобразовательных учреждений, чел</td>
				<td>Выпущено учеников общеобразовательных учреждений, чел</td>
				<td>Кол-во учеников общеобразовательных учреждений, чел</td>
				<td>Кол-во преподавателей, чел</td>
				<td>Инвестиции в образование</td>
				<td>Количество дневных общеобразовательных учебных заведений, ед</td>
				<td>Кол-во учеников на одно общеобразовательное учреждение, чел</td>
				<td>Кол-во учителей на одно общеобразовательных учреждений, чел</td>
				<td>Кол-во проф-тех учреждений</td>
				<td>Кол-во учащизхся в проф-тех учреждениях, чел</td>
				";
				while ($out = mysql_fetch_array($result,MYSQL_ASSOC)){
				echo "<tr>";
				$a = 0;
